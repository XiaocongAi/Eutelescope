		...
		meas[1] = state.getHit().getPosition()[1] - state.getPosition()[1];
	        ...
	        // streamlog_out(MESSAGE5) <<"Hit position for plane "<<state.getLocation()<<" is " << state.getHit().getPosition()[0]<<", "<<state.getHit().getPosition()[1] <<std::endl;
		        ...
		        //streamlog_out(MESSAGE5) <<"Hit position for plane "<<state.getLocation()<<" is " << state.getHit().getPosition()[0]<<", "<<state.getHit().getPosition()[1] <<"r = "<<r_meas<<" with cosang = "<<cos(ang_meas)<<" sinang = "<<sin(ang_meas)<<"res is "<<meas[0]<<"   "<<meas[1]<<std::endl;
                       ...
                       //covxy[1][0] = (cov[0]-cov[3])*sin(ang_meas)*cos(ang_meas);
                       //covxy[1][1] = cov[0]*sin(ang_meas)*sin(ang_meas) + cov[3]*cos(ang_meas)*cos(ang_meas);

                        covxy[0][0] = cov[0]*cos(ang_meas)*cos(ang_meas) + cov[3]*r_meas*r_meas*sin(ang_meas)*sin(ang_meas);
 	       		covxy[0][1] = (cov[0]-cov[3]*r_meas*r_meas)*sin(ang_meas)*cos(ang_meas);
			covxy[1][0] = (cov[0]-cov[3]*r_meas*r_meas)*sin(ang_meas)*cos(ang_meas);
...
//                        std::cout<<"MeasRes : "<<meas[0]<<" : "<<meas[1]<<" trans Res "<<transResiduals[0]<<" : "<<transResiduals[1]<<" with res "<<sqrt(1.0/transPrecision[0])<<" : "<<sqrt(1.0/transPrecision[1])<<std::endl; 
			...
			//streamlog_out(MESSAGE5) <<" X:" << std::setw(20) << meas[0] << std::setw(20) << Precxy[0][0] <<"," << std::endl;
			//streamlog_out(MESSAGE5) << "Y:" << std::setw(20) << meas[1] << std::setw(20)  <<"," << Precxy[1][1] << std::endl;
